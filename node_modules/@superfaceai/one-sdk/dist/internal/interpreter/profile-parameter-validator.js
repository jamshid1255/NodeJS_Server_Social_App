...
var debug = debug_1.default('superface:profile-parameter-validator');
...
var ProfileParameterValidator = /** @class */ (function () {
    function ProfileParameterValidator(ast) {
        this.ast = ast;
    ...
    ProfileParameterValidator.prototype.validate = function (input, kind, usecase) {
        try {
            ...
            var _a = __read(validator(input), 2), result = _a[0], errors = _a[1];
            if (result !== true) {
                debug("Validation of " + kind + " failed with error(s):\n" + profile_parameter_validator_errors_1.formatErrors(errors));
            ...
            return lib_1.err(new errors_1.UnexpectedError('Unknown error from validator', e));
        }
    };
    ...
    ProfileParameterValidator.prototype.visitFieldDefinitionNode = function (node, kind, usecase) {
        var _this = this;
        return function (input) {
            var field = objectHasKey(input, node.fieldName)
                ? input[node.fieldName]
                : undefined;
            if (!node.type) {
                if (_this.namedFieldDefinitions[node.fieldName]) {
                    return _this.namedFieldDefinitions[node.fieldName](field);
                }
                return [true];
            }
            ...
            return _this.visit(node.type, kind, usecase)(field);
        };
    };
    ...
    ProfileParameterValidator.prototype.visitModelTypeNameNode = function (node, _kind, _usecase) {
        ...
        throw new errors_1.UnexpectedError("Invalid model name: " + node.name);
    };
    ...
    ProfileParameterValidator.prototype.visitProfileDocumentNode = function (node, kind, usecase) {
        ...
        if (!usecaseNode) {
            throw new errors_1.UnexpectedError("Usecase " + usecase + " not found!");
        }
