...
exports.computeHash = exports.extractQop = exports.extractAlgorithm = exports.DigestHandler = void 0;
...
var debug = debug_1.default('superface:http:security:digest-handler');
var debugSensitive = debug_1.default('superface:http:security:digest-handler:sensitive');
...
var DigestHandler = /** @class */ (function () {
    function DigestHandler(configuration, fetchInstance) {
        var _this = this;
        var _a;
        ...
        this.authenticate = function (parameters) { return __awaiter(_this, void 0, void 0, function () {
            var headers, response, credentials;
            var _a;
            var _b, _c;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        if ((_b = this.fetchInstance) === null || _b === void 0 ? void 0 : _b.digest) {
                            debugSensitive('Using cached digest credentials');
                            headers[this.configuration.authorizationHeader ||
                                interfaces_1.DEFAULT_AUTHORIZATION_HEADER_NAME] = this.fetchInstance.digest;
                            return [2 /*return*/, __assign(__assign({}, parameters), { headers: headers })];
                        }
                        return [4 /*yield*/, pipe_1.pipe({
                                parameters: __assign(__assign({}, parameters), { headers: headers }),
                            }, filters_1.prepareRequestFilter, filters_1.withRequest(filters_1.fetchFilter(this.fetchInstance)))];
                    case 1:
                        ...
                        if (response.statusCode !== this.statusCode ||
                            ...
                            throw errors_helpers_1.digestHeaderNotFound(this.challengeHeader, Object.keys(response.headers));
                        }
                        debugSensitive('Getting new digest values');
                        credentials = this.buildDigestAuth(
                        // We need actual resolved url
                        response.debug.request.url, parameters.method, this.extractDigestValues(response.headers[this.challengeHeader]));
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        ...
                        credentials = this.buildDigestAuth(
                        // We need actual resolved url
                        response.debug.request.url, resourceRequestParameters.method, this.extractDigestValues(response.headers[this.challengeHeader]));
                        this.fetchInstance.digest = credentials;
        ...
        this.authorizationHeader =
            (_a = configuration.authorizationHeader) !== null && _a !== void 0 ? _a : interfaces_1.DEFAULT_AUTHORIZATION_HEADER_NAME;
        debugSensitive("Initialized with: username=\"" + this.configuration.username + "\", password=\"" + this.configuration.password + "\", status code=" + this.statusCode + ", challenge header=\"" + this.challengeHeader + "\", authorization header=\"" + this.authorizationHeader + "\"");
    }
    /**
     *
    ...
    DigestHandler.prototype.buildDigestAuth = function (url, method, digest) {
        debugSensitive("Preparing digest authentication for: " + url + " and method: " + method);
        var uri = new URL(url).pathname;
        // Default H1
        var ha1 = computeHash(digest.algorithm, this.configuration.username + ":" + digest.realm + ":" + this.configuration.password);
        // sess H1 contains original H1 and also nonce and cnonce
        if (digest.algorithm.endsWith('-sess')) {
            ha1 = computeHash(digest.algorithm, ha1 + ":" + digest.nonce + ":" + digest.cnonce);
        }
        // H2 is same for default and sess
        var ha2 = computeHash(digest.algorithm, method + ":" + uri);
        this.nc++;
        ...
        // Hash response
        var hashedResponse = computeHash(digest.algorithm, response);
        // Build final auth header
        var opaqueString = digest.opaque ? "opaque=\"" + digest.opaque + "\"" : '';
    ...
    DigestHandler.prototype.extractDigestValues = function (header) {
        debugSensitive("Extracting digest authentication values from: " + header);
        var scheme = header.split(/\s/)[0];
        if (!scheme) {
            throw errors_helpers_1.missingPartOfDigestHeader(this.challengeHeader, header, 'scheme');
        }
        var nonce = extract(header, 'nonce');
        if (!nonce) {
            throw errors_helpers_1.missingPartOfDigestHeader(this.challengeHeader, header, 'nonce');
        }
        return {
            scheme: scheme,
    ...
    DigestHandler.prototype.makeNonce = function () {
        ...
        for (var i = 0; i < this.cnonceSize; ++i) {
            uid += this.nonceRaw[crypto_1.randomInt(this.nonceRaw.length)];
        }
        return uid;
    };
...
function extractAlgorithm(rawHeader) {
    ...
    if (extractedValue !== undefined) {
        if (extractedValue === 'MD5') {
            ...
            throw errors_helpers_1.unexpectedDigestValue('algorithm', extractedValue, [
                'MD5',
                'MD5-sess',
                'SHA-256',
                'SHA-256-sess',
...
function extractQop(rawHeader) {
    ...
    if (parsedQop !== undefined) {
        ...
        if (qops.includes('auth-int')) {
            ...
            throw errors_helpers_1.unexpectedDigestValue('quality of protection', qops.join(', '), [
                'auth',
                'auth-int',
            ]);
        }
...
function computeHash(algorithm, data) {
    ...
    return crypto_1.createHash(usedAlgorithm).update(data).digest('hex');
}
