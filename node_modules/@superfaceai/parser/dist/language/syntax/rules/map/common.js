 ...
 * if (<jessie>)
 */
exports.CONDITION_ATOM = rule_1.SyntaxRule.followedBy(rule_1.SyntaxRule.identifier('if'), rule_1.SyntaxRule.separator('('), JESSIE_EXPRESSION_FACTORY(')'), rule_1.SyntaxRule.separator(')')).map(function (_a) {
...
exports.ITERATION_ATOM = rule_1.SyntaxRule.followedBy(rule_1.SyntaxRule.identifier('foreach'), rule_1.SyntaxRule.separator('('), rule_1.SyntaxRule.identifier(), rule_1.SyntaxRule.identifier('of'), JESSIE_EXPRESSION_FACTORY(')'), rule_1.SyntaxRule.separator(')')).map(function (_a) {
...
var PROFILE_ID = rule_1.SyntaxRule.followedBy(rule_1.SyntaxRule.identifier('profile'), rule_1.SyntaxRuleSeparator.operator('='), rule_1.SyntaxRule.string().andThen(function (id) {
    var parseIdResult = (0, parser_1.parseProfileId)(id.data.string);
...
}, 'profile id in format `[<scope>/]<name>@<semver>` with lowercase identifiers')).map(function (_a) {
...
var PROVIDER_ID = rule_1.SyntaxRule.followedBy(rule_1.SyntaxRule.identifier('provider'), rule_1.SyntaxRuleSeparator.operator('='), rule_1.SyntaxRule.string().andThen(function (provider) {
    if (!(0, ast_1.isValidDocumentName)(provider.data.string)) {
        return {
            kind: 'nomatch',
        };
    }
...
}, 'lowercase identifier')).map(function (_a) {
    var _b = __read(_a, 3), keyword = _b[0], _op = _b[1], provider = _b[2];
    return {
        provider: provider.provider,
        location: (0, common_1.computeLocationSpan)(keyword, provider),
    };
});
exports.MAP_VARIANT = rule_1.SyntaxRule.followedBy(rule_1.SyntaxRule.identifier('variant'), rule_1.SyntaxRuleSeparator.operator('='), rule_1.SyntaxRule.string().andThen(function (variant) {
    if (!(0, ast_1.isValidDocumentName)(variant.data.string)) {
        return {
            kind: 'nomatch',
        };
    }
    ...
    return {
        ...
        location: (0, common_1.computeLocationSpan)(keyword, variant),
    };
});
exports.MAP_HEADER = (0, common_1.documentedNode)(rule_1.SyntaxRule.followedBy(PROFILE_ID, PROVIDER_ID, rule_1.SyntaxRule.optional(exports.MAP_VARIANT)).map(function (_a) {
    var _b, _c;
    ...
    return {
        ...
        location: (0, common_1.computeLocationSpan)(profile, provider, maybeVariant),
    };
}));
/**
