...
function processScriptText(scriptText) {
    var compiler = new script_1.ScriptExpressionCompiler(scriptText);
    var errors = compiler.diagnose().map(jessie_1.mapScriptDiagnostic);
    if (errors.length === 0) {
        errors = compiler.validate().map(jessie_1.mapValidatorDiagnostic);
    }
    if (errors.length > 0) {
        return {
            kind: 'error',
    ...
    var _a = compiler.transpile(), output = _a.output, sourceMap = _a.sourceMap;
    return {
        kind: 'match',
        data: {
