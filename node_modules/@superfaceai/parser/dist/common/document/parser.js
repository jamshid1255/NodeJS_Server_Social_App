...
function tryParseVersionNumber(str) {
    try {
        return (0, ast_1.parseVersionNumber)(str);
    }
    catch (error) {
        return undefined;
    }
...
function parseDocumentId(id) {
    ...
    // parse scope first
    var scope;
    var _b = __read((0, ast_1.splitLimit)(id, '/', 1), 2), splitScope = _b[0], scopeRestId = _b[1];
    if (scopeRestId !== undefined) {
        scope = splitScope;
        if (!(0, ast_1.isValidDocumentName)(scope)) {
            return {
                kind: 'error',
                message: "".concat(scope, " is not a valid lowercase identifier"),
            };
        }
        // strip the scope
        id = scopeRestId;
    ...
    var parsedVersion;
    var _c = __read((0, ast_1.splitLimit)(id, '@', 1), 2), versionRestId = _c[0], splitVersion = _c[1];
    if (splitVersion !== undefined) {
        try {
            ...
            return {
                kind: 'error',
                message: "".concat(splitVersion, " is not a valid version"),
            };
        }
        // strip the version
        id = versionRestId;
    ...
    try {
        for (var middle_1 = __values(middle), middle_1_1 = middle_1.next(); !middle_1_1.done; middle_1_1 = middle_1.next()) {
            ...
            if (!(0, ast_1.isValidDocumentName)(m)) {
                return {
                    kind: 'error',
                    message: "\"".concat(m, "\" is not a valid lowercase identifier"),
                };
            }
        }
    }
...
function parseProfileId(id) {
    ...
    if (base.middle.length !== 1) {
        return {
            kind: 'error',
            message: "\"".concat(base.middle.join('.'), "\" is not a valid lowercase identifier"),
        };
    }
